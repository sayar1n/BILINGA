...
namespace backend
{
    public class TGbot
    {
        ...
        public async Task Start()
        {
            ...
            {
                AllowedUpdates = Array.Empty<UpdateType>() // Только команды
            };
            ...
            Console.WriteLine("Бот запущен с токеном: " + BotToken);
        ...
        private async Task SendNotifications()
        {
            ...
            foreach (var userId in users)
            {
                try
                {
                    await botClient.SendMessage(userId, "Ты уже позанимался английским? Пора сделать это!!");
                }
                catch (Exception ex)
                {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (message.Text!.ToLower() == "/start")
            {
                ...
                await botClient.SendMessage(chatId,
                    "Привет! Теперь BILINGA поможет не забыть позаниматься английским.");
            }
            else if (message.Text.ToLower() == "/stop")
            {
                RemoveUser(chatId);
                await botClient.SendMessage(chatId, "Ты отписался от напоминаний.");
            }
        }
        ...
        {
            if (!File.Exists(UsersFile)) return new HashSet<long>();
